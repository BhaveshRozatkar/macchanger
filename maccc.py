...
def get_adapters():
    # Return list of (name, mac) tuples
...
ttk.Label(main, text="Select Interface:").grid(row=0, column=0, sticky='w')
adapter_menu = ttk.Combobox(main, textvariable=adapter_var, values=displays, state='readonly', width=30)
...
random_rb.grid(row=1, column=0, columnspan=2, sticky='w')
manual_rb.grid(row=2, column=0, columnspan=2, sticky='w')

# Manual entry
ttk.Label(main, text="MAC Address:").grid(row=3, column=0, sticky='w')
manual_entry = ttk.Entry(main, width=30)
...
status_label = ttk.Label(main, textvariable=status_var, wraplength=300)
